...
# --- Neues Log-Fenster ---
    ...
    def setup_docker_tab(self):
        ...
        self.create_modern_btn(rt_cat, self.t("docker.catalog"), self.open_docker_catalog, getattr(self, "color_btn_dark", self.color_btn_secondary)).pack(**_dock_l)
        self._register_danger_rounded(self.create_modern_btn(rt_cat, self.t("docker.new"), self.open_docker_creator, "#3b82f6")).pack(**_dock_l)

        rt_ex_up_li = tk.Frame(tool_top, bg=self.color_surface)
        rt_ex_up_li.pack(fill=tk.X, pady=(6, 0))
        self.create_modern_btn(rt_ex_up_li, self.t("docker.exclude_list"), self.open_docker_exclusion_dialog, self.color_btn_secondary).pack(**_dock_l)
        self._register_danger_rounded(self.create_modern_btn(rt_ex_up_li, self.t("docker.update"), self.docker_update_selected, self.color_btn_blue)).pack(**_dock_l)
        self.create_modern_btn(rt_ex_up_li, self.t("docker.list"), self.refresh_docker_list, self.color_btn_secondary).pack(**_dock_l)

        rt_ssr_li = tk.Frame(tool_top, bg=self.color_surface)
        rt_ssr_li.pack(fill=tk.X, pady=(6, 0))
        self._register_danger_rounded(self.create_modern_btn(rt_ssr_li, self.t("docker.start"), lambda: self.docker_action("start"), self.color_user)).pack(**_dock_l)
        self._register_danger_rounded(self.create_modern_btn(rt_ssr_li, self.t("docker.stop"), lambda: self.docker_action("stop"), self.color_root)).pack(**_dock_l)
        self._register_danger_rounded(
            self.create_modern_btn(rt_ssr_li, self.t("docker.restart"), lambda: self.docker_action("restart"), self.color_cron, "white")
        ).pack(**_dock_l)
        self.create_modern_btn(rt_ssr_li, self.t("docker.list"), self.refresh_docker_list, self.color_btn_secondary).pack(**_dock_l)

        rt_stop_all = tk.Frame(tool_top, bg=self.color_surface)
        rt_stop_all.pack(fill=tk.X, pady=(6, 0))
        self._register_danger_rounded(self.create_modern_btn(rt_stop_all, self.t("docker.stop_all"), self.docker_stop_all, self.color_root)).pack(**_dock_l)
