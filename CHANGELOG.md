...
- **Windows-Installer → erste App-Sprache:** Die im **Inno-Setup** gewählte Sprache wird nach der Installation als **`InstallerUiLang`** unter **`HKCU\Software\UgreenNASAdmin`** gespeichert. Beim **ersten Start** übernimmt die App diesen Code als **`ui_lang`**, schreibt ihn nach **`nas_admin_connection.json`** und **löscht den Registry-Wert** (bestehende `ui_lang` in der Datei bleibt unverändert).
